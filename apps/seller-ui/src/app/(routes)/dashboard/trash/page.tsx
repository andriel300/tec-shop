...
const TrashPage = () => {
  ...
  const { data: deletedProducts, isLoading, isError } = useDeletedProducts({
    search: searchQuery || undefined,
  });
  const { mutate: restoreProductMutation, isPending: isRestoring } = useRestoreProduct();
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-yellow-200 font-medium">Recovery Window Active</p>
              <p className="text-yellow-300/80 text-sm mt-1">
                Products in trash can be restored within 24 hours of deletion. After that, they will be permanently removed and cannot be recovered.
              </p>
            </div>
          </div>
        </div>
        ...
        {isError && (
          <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-6 text-center">
            ...
            <h3 className="text-xl font-semibold text-red-300 mb-2">Error Loading Trash</h3>
            <p className="text-red-200/80">Failed to load deleted products. Please try again.</p>
          </div>
        )}

        {/* Empty State */}
        {!isLoading && !isError && (!deletedProducts || deletedProducts.length === 0) && (
        ...
        {deletedProducts && deletedProducts.length > 0 && (
          <div className="mt-6 text-center">
            <p className="text-gray-400 text-sm">
              Showing {deletedProducts.length} deleted {deletedProducts.length === 1 ? 'product' : 'products'}
            </p>
          </div>
        )}
      </div>
