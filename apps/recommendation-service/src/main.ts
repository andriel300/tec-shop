...
initializeSentryForService('recommendation-service', process.env['RECOMMENDATION_SERVICE_PORT'] ?? '6009');
...
async function bootstrap() {
  ...
  const tlsOptions = {
    key: readFileSync(join(certsPath, 'recommendation-service/recommendation-service-key.pem')),
    cert: readFileSync(join(certsPath, 'recommendation-service/recommendation-service-cert.pem')),
  ...
  const metricsPort = parseInt(process.env.RECOMMENDATION_METRICS_PORT ?? '9010', 10);
  await app.listen(metricsPort, '0.0.0.0');
  Logger.log(`recommendation-service TCP on port ${process.env.RECOMMENDATION_SERVICE_PORT ?? 6009} with mTLS, metrics on port ${metricsPort}`);
}
